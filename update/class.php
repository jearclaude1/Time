...
$id=$_GET['class_id'];
$delete =mysqli_query($db," SELECT * FROM `class` WHERE class_id='$id' ");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="content">
        ...
        <form action="" method="post">
            <?php
                   ...
                   while($row= mysqli_fetch_assoc($delete)) {
                       ?>
                      <input type="text" name="name" value="<?=$row['class_name']?>" placeholder="Course_name">
                      <input type="text" name="detail" value="<?=$row['class_detail']?>" placeholder="Course_name">
        ...
        if (isset($_POST['save'])) {
            $name=$_POST['name'];
            $detail=$_POST['detail'];
            $query=mysqli_query($db," UPDATE `class` SET `class_name`='$name',`class_detail`='$detail' WHERE class_id='$id' ");
            if ($query ==TRUE) {
